...
- We have already shown that headlines from some outlets are longer (see [Ntoken](#ntoken)), but we could go deeper into this and try to investigate how the **complexity of language** differs between outlets of ``negative``/ ``NotNegative`` classified headlines by calculating **Readability scores**

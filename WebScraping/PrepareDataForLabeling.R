...
setwd("C:/Users/nadin/OneDrive/Dokumente/Quantitative_Text_Analysis/Project/Code/Data")
...
#labels 
labels <- read_xlsx("HeadlinesLabeled.xlsx", sheet ="headlines")
labels <- labels[c(3,12)]
colnames(labels) <- c("id","human_coding")

#set 1 to Negative and = to NotNegative
labels$human_coding[labels$human_coding == 1] <- "Negative"
labels$human_coding[labels$human_coding == 0] <- "NotNegative"

#make it as factor
labels$human_coding <- as.factor(labels$human_coding)

table(labels$human_coding)
